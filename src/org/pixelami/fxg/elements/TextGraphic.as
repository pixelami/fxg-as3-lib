...
package org.pixelami.fxg.elements
{
	...
	{
		private var _fontFamily:String = "Arial"; // <String>: The font family name used to render the text. If no font is supplied, the client will pick one that is a variant of the Times family, dependent on platform. Which font is used for rendering the text is up to the client and also depends on the glyphs that are being rendered and the fonts that are available. Default value is Times New Roman (Times on Mac OS X).
		private var _fontSize:Number = 12; // <Number>: The size of the glyphs that is used to render the text, specified in point sizes. Default is 12. Minimum 1 point. Maximum 500 points.
		private var _fontStyle:String = "normal"; // <String> (normal, italic): The style of the glyphs that is used to render the text. Legal values are 'normal' and 'italic'. Default is normal.
		private var _fontWeight:String = "normal"; /// <String>(normal, bold): The boldness or lightness of the glyphs that is used to render the text. Default is normal.
		private var _lineHeight:Number = 120; // <Percent> | <Number>: The leading, or the distance from the previous line's baseline to this one, in points. This can be specified in absolute pixels, or as a percentage of the point size. Default is 120%. Minimum value for percent or number is 0.
		private var _tracking:Number = 0; // <Percent>: Space added to the advance after each character, as a percentage of the current point size. Percentages can be negative, to bring characters closer together. Default is 0.
		private var _textDecoration:String = "none"; // <String> (none, underline): The decoration to apply to the text. Default is none.
		private var _lineThrough:Boolean = false; // <Boolean>: true if text has strikethrough applied, false otherwise. Default is false.
		private var _color:uint = 0x000000; // <Color>: The color of the text. Default is 0x000000.
		private var _textAlpha:Number = 1 // <alpha>: The alpha value applied to the text. Default is 1.0.
		private var _whiteSpaceCollapse:Number ; // <String> (preserve, collapse): This is an enumerated value. A value of "collapse" converts line feeds, newlines, and tabs to spaces and collapses adjacent spaces to one. See [Text collapse examples] for details on collapse handling. Updated:-Leading and trailing whitespace is trimmed. A value of "preserve" passes whitespace through unchanged,except as noted earlier, when the whitespace would result in an implied <p> and <span> that is all whitespace, in which case the whitespace is removed.
		private var _lineBreak:String = "toFit"; // <String> (toFit, explicit): This is an enumeration. A value of "toFit" wraps the lines at the edge of the enclosing TextGraphic. A value of "explicit" breaks the lines only at a Unicode line end character (such as a newline or line separator). toFit is the default.
		private var _kerning:String = "auto"; // <String> (on, off, auto): If on, pair kerns are honored. If off, there is no font-based kerning applied. If auto, kerning is applied to all characters except Kanji, Hiragana or Katakana. The default is auto.
		...
		*/
		...
		private function createLines(textBlock:TextBlock):void 
		{
			...
			if(w) 
			{
				trace("creating line with "+w+" width");
				textLine = textBlock.createTextLine (null,w);
				
			}
			else
			{
				trace("creating line with default width");
				textLine = textBlock.createTextLine (null);
			}
			
			trace("TextGraphic w",w);
			trace("textLine.textWidth",textLine.textWidth);
			trace("textLine.width",textLine.width);
